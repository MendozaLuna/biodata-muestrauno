...
<style>
    ...
    /* Contenedor del mapa con bordes redondeados */
    .map-container {
        border-radius: 20px;
        overflow: hidden;
...
def generar_copy_oferta(estudio, precio):
    model = genai.GenerativeModel('models/gemini-flash-latest')
    res = model.generate_content(f"Copy persuasivo para {estudio} a ${precio}. Con emojis.")
    return res.text
...
def analizar_imagen_ai(img_bytes):
    ...
    return partes[0].strip().upper(), partes[1].strip() if len(partes) > 1 else "Estudio ocular."
...
if st.session_state.perfil is None:
    ...
    if df_red is not None:
        ...
        for _, row in df_red.iterrows():
            folium.Marker([float(row['Latitud']), float(row['Longitud'])], popup=row.get('Nombre de la Clinica', 'Sede'), icon=folium.Icon(color='cadetblue', icon='hospital', prefix='fa')).add_to(m_red)
        
        c_izq, c_cen, c_der = st.columns([1, 10, 1])
        with c_cen:
...
if st.session_state.perfil == 'persona':
    ...
    if st.button("🚀 BUSCAR MEJORES OPCIONES"):
        try:
            ...
            # Lógica de búsqueda simplificada para velocidad
            ...
            if not res_df.empty:
                ...
                mejor = res_df.sort_values(by='Precio').iloc[0]
...
# --- 7. CONTENIDO EMPRESA ---
    ...
    if clave in ACCESOS_CLINICAS:
        ...
        tab_stats, tab_oferta, tab_inv = st.tabs(["📊 Estadísticas", "⚡ Oferta IA", "🛠️ Inventario"])
        
        with tab_inv:
            eq = st.selectbox("Equipo:", ["OCT", "Campímetro", "Láser"], key="eq_emp")
            st_eq = st.radio("Estatus:", ["Operativo", "En Mantenimiento"])
            if st.button("Actualizar"):
                try:
                    supabase.table("inventario_equipos").insert({"clinica": nombre_c, "equipo": eq, "estado": st_eq, "ultima_actualizacion": datetime.now().isoformat()}).execute()
