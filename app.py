        ...
        try:
            ...
            if not resultados.empty:
                ...
                with c_info:
                    ...
                    st.info(f"📅 Precios verificados: {fecha_hoy}")

                    # Premium
                    for _, row in premium_df.iterrows():
                        wa = str(int(row['Whatsapp'])) if pd.notna(row['Whatsapp']) else ""
                        url = f"https://wa.me/{wa}?text=Hola!%20Deseo%20agendar%20{nombre_estudio}%20en%20{row['Nombre']}"
                        st.markdown(f'<div class="premium-card"><p>💎 PREMIUM</p><h3>{row["Nombre"]}</h3><h2>${int(row["Precio"])}</h2><p>📍 {row["Km"]} km</p><a href="{url}" class="btn-whatsapp" target="_blank">💬 AGENDAR</a></div>', unsafe_allow_html=True)

                    # Recomendado
                    if not basic_df.empty:
                        m = basic_df.iloc[0]
                        wa_m = str(int(m['Whatsapp'])) if pd.notna(m['Whatsapp']) else ""
                        url_m = f"https://wa.me/{wa_m}?text=Hola!%20Precio%20de%20{nombre_estudio}%20en%20{m['Nombre']}"
                ...
                st.error(f"No hay sedes para '{nombre_estudio}'. Intenta con una palabra clave en el buscador manual.")
